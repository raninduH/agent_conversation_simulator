...
class SimulationTab(ttk.Frame):
    ...
    def display_message(self, message_data, blinking=False):
        ...
        # Track all loading and non-loading bubbles (as lists)
        if not hasattr(self, '_loading_bubbles'):
            self._loading_bubbles = []  # list of message_data dicts
        if not hasattr(self, '_non_loading_bubbles'):
            self._non_loading_bubbles = []

        # Remove loading bubble(s) for the same agent if a non-loading message arrives from that agent
        def remove_loading_by_agent(agent_id, agent_name, sender):
            self._loading_bubbles = [m for m in self._loading_bubbles if not (
                (agent_id and m.get('agent_id') == agent_id) or
                (agent_name and m.get('agent_name') == agent_name) or
                (sender and m.get('sender') == sender)
            )]

        # Always store color and align_right in the message dict
        message_data = dict(message_data)  # avoid mutating caller's dict
        message_data['color'] = color
        message_data['align_right'] = align_right
    ...
    def handle_message_callback(self, message_data):
        ...
        if isinstance(message_data, dict):
            ...
            if action == "stop_blinking":
                ...
                # Set blinking=False for all non-loading chat bubbles and stop their blinking
                if hasattr(self, '_non_loading_bubbles'):
